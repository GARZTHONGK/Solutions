...
def get_record(classparam, record_id):  # classparam is the table i think, record_id is the cell id
    with Session(engine) as session:
...
def delete_soft_container(container):
    with Session(engine) as session:
        session.execute(update(Container).where(Container.id == container).values(weight=-1, destination=container.destination))
        session.commit()
